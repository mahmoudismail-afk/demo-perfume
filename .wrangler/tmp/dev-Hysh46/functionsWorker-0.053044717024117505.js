...
// .wrangler/tmp/bundle-Y0yLmJ/checked-fetch.js
...
// .wrangler/tmp/pages-8wmsUr/functionsWorker-0.053044717024117505.mjs
...
var withAuth = /* @__PURE__ */ __name2((handler) => async (context) => {
  ...
  const token = authHeader.split(" ")[1];
  try {
    const isValid = await index_default.verify(token, env.JWT_SECRET);
    if (!isValid) {
...
var onRequestPut = withAuth(async (context) => {
  ...
  try {
    ...
    `;
    const { success } = await env.DB.prepare(query).bind(status, orderId, tenantId).run();
    if (!success) {
...
var onRequestPost4 = withAuth(async (context) => {
  ...
  try {
    ...
    `;
    const { results, success } = await env.DB.prepare(updateQuery).bind(tenantId, promoCode).all();
...
// .wrangler/tmp/bundle-Y0yLmJ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Y0yLmJ/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.053044717024117505.js.map
